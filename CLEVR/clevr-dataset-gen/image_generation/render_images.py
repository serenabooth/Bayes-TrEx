...
parser.add_argument('--obj_properties',
    help="A JSON file containing object proprt")
...
def main(args):
  ...
  for i in range(args.num_images):
    ...
    obj_properties = args.obj_properties
    assert (obj_properties != None)

    success_flag = render_deterministic_scene(args,
    ...
    if not success_flag:
      all_scene_paths.remove(scene_path)
...
def render_deterministic_scene(args,
    num_objects=5,
    object_properties={},
    output_index=0,
    output_split='none',
  ...
  """
  Render a scene according to the object_properties dictionary
  """

  if ".json" in object_properties:
    with open(object_properties, 'r') as f:
      object_dict = json.load(f)
      num_objects = object_dict["num_objects"]
      print (object_dict)
  else:
    object_dict = object_properties
  ...
  objects, blender_objects = add_specific_objects(scene_struct, object_dict, num_objects, args, camera)
  ...
  if objects != None:
    ...
    # Failure - silent (just don't write the file)
    return False
