...
def program_all_but_one(nn_model, label, std=0.05):
	'''
...
def setting(args):
	folder = 'results/adversarial/%s'%args.dataset
	os.makedirs(folder, exist_ok=True)
	save_fn = '%s/%s_wo_%i.txt'%(folder, args.nn_model, args.excl_label)
	...
	if not os.path.isfile(save_fn):
		...
		fo.write('\n')
		fo.close()
	path = 'saved_models/all_but_one/%s_%s_wo_%i.pth'%(args.dataset, args.nn_model, args.excl_label)
	if args.nn_model == 'vae':
		nn_model = VAEModel(args.dataset, vae_path=path)
