...
parser = argparse.ArgumentParser(description='VAE FashionMNIST Example')
...
class FashionMNISTExclDataset(Dataset):
	def __init__(self, excl_label, root, train=True, download=False, 
		transform=None, target_transform=None):

		self.excl_label = excl_label
		fashion_mnist = datasets.FashionMNIST(root, train=train, download=download, 
			transform=transform, target_transform=target_transform)
		self.xs = []
		self.ys = []
		for i in xrange(len(fashion_mnist)):
			x, y = fashion_mnist[i]
			if y != excl_label:
				self.xs.append(x)
				self.ys.append(y)
...
def vae_without(digit):
	train_loader = torch.utils.data.DataLoader(
		FashionMNISTExclDataset(digit, './data', train=True, download=True, 
			transform=transforms.ToTensor()), 
		batch_size=args.batch_size, shuffle=True, **kwargs)
	test_loader = torch.utils.data.DataLoader(
		FashionMNISTExclDataset(digit, './data', train=False, download=True, 
			transform=transforms.ToTensor()), 
		batch_size=args.batch_size, shuffle=True, **kwargs)
